...
const result = {
  comments: [{ plain_body: "comment 1" }, { plain_body: "comment 2" }]
};
...
describe("Zendesk Webhook", () => {
  describe("Given an update request is fired", () => {
    ...
    it("should get the zendesk ticket comments", async () => {
      ...
      await expect(
        getTicketComments.handler({ config, data, database: {} })
      ).resolves.toBeUndefined();
      expect(utils.runZendeskOperation).toBeCalledWith(
        config.crm,
        `/tickets/${data.id}/comments.json?include=users`
      );
      expect(spyFormatOutput).toBeCalledWith(config, data, result.comments);
